 ...
 Properties
    {
        ...
        _Stickiness("Stickiness", float) = 0
        _Side("side", float) = 0
        ...
        half _SetSize, _hit, _size, _inAngle, _Stickiness, _Side, _Ref;
        ...
        v2f vert(appdata v)
        {
            ...
            half3 posC = _posC.xzy;  //Followに追従する先端の位置。
            posC.x *= ((1 / _SetSize) + .1); //サイズ変更でz方向に平筆の形は変えないのでxにだけ乗算
            ...
            //posCと対応させる
            half2 curve = half2(posC.x * vc.r, -posC.z * vc.r);
            //curve = lerp(curve * (clamp(st.x - .3, 0, 1.5 * _SetSize)), curve, offset); //細い筆がばらけすぎないよう
            curve = lerp(curve, curve * .5, _inAngle); //傾いている筆が曲がりすぎないよう。
            curve.y *= 2; //ちょっと曲がり方を激しく
            curve *= lerp(st.x, 1 - st.x, _Side); //左右のどちらかに偏ったカーブ。
            vertex.xz += curve;
            ...
            */
            ...
            /*
            //筆の開き
            half2 open = lerp(0, modifiedNormal.xz * .03, clamp(offset - .2, 0, 1));
            //開いた時の座標を頂点カラーに応じて決定
            ...
            */
            ...
            //筆が倒れているときはあまり動かさない。
            //vertex.y -= lerp(0, lerp(0, lerp(.035, 0, _inAngle), _SetSize), offset) * vc.r;
